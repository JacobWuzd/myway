...
- [ ] 基本配置
  ...
  - output：出口
    ...
    ```js
    const path = require('path'); // 引入node的path模块，来操作文件路径
    
    module.exports = {
      entry: './path/to/my/entry/file.js',
      output: {
        // __dirname获得当前文件完整路径
        path: path.resolve(__dirname, 'dist'), // 输出到文件夹dist中
  ...
  - mode：模式，development或production，分别表示开发模式和生成模式
...
- [ ] webpack优化
  ...
  - babel缓存：cacheDirectory: true，让第二次打包构建速度更快
    ...
    - 问题：如果缓存期间有bug，也不会加载新的资源，直到缓存期结束才会加载新的资源

  - 文件资源缓存：

    - hash：每次webpack构件式会生成一个唯一hash值，可以解决强缓存期间如果有bug修复后不重新加载资源的问题
      - 问题：如果只修改了一个文件，在重新打包时，所有文件的hash都会改变，导致所有缓存失效
    - chunkhash：根据chunk生成的hash值，如果打包来源于同一个chunk，那么hash值就一样，解决hash在重新打包时导致所有缓存失效的问题
      - 问题：js和css的hash值还是一样的，因为css是在js中引入的，所以属于同一个chunk
  ...
  - 优化流程：
    ...
    - 优化loader配置：
      ...
      1. 优化正则匹配，如果项目中没有用到的文件类型就不参与匹配
      2. babel缓存，给babel-loader加上cacheDirectory:true
      3. 只对项目src目录想的文件采用babel-loader
      4. 用alias属性去将原导入路径映射成一个新的导入路径，减少耗时的递归解析操作
      5. noParse：'/jquery|lodash/'，让webpack忽略部分没采用模块化文件的递归解析

    - DLL plugins结合extrenals：不打包第三方库

    - 采用thread-loader进行多进程打包
